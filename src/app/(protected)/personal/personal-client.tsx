    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        ...
        {initialData.canCreate && (
          <button type="button" onClick={openCreate} className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700">
      ...
      {undefinedWorkerTypeCount > 0 && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
          Hay {undefinedWorkerTypeCount} trabajador{undefinedWorkerTypeCount === 1 ? "" : "es"} sin tipo definido. Edítalos para indicar Full Time o Part Time.
        </div>
      )}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="grid gap-3 border-b border-slate-200 p-4 xl:grid-cols-[1fr_190px_180px_170px]">
          <label className="relative block">
            ...
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nombre, correo, DNI, tienda o tipo..." className="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-10 pr-3 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
          </label>
          <select value={roleFilter} onChange={(event) => setRoleFilter(event.target.value as "all" | AppRole)} className="rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
          ...
          <select value={workerTypeFilter} onChange={(event) => setWorkerTypeFilter(event.target.value as "all" | "undefined" | WorkerType)} className="rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
            <option value="all">Todos los tipos</option>
            <option value="full_time">Full Time</option>
            <option value="part_time">Part Time</option>
            <option value="undefined">Sin definir</option>
          </select>
          <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as "all" | "active" | "inactive")} className="rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
          ...
          <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
                ...
                <tr key={record.id} className="align-top hover:bg-slate-50/70">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {record.isSelf && <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-semibold text-blue-700">Tú</span>}
                        </div>
                  ...
                  <td className="px-4 py-4">
                    {record.role && record.position ? (
                      <div><p className="font-medium text-slate-800">{ROLE_LABELS[record.role]}</p><p className="mt-1 text-xs text-slate-500">{POSITION_LABELS[record.position]}</p></div>
                    ) : <span className="rounded-full bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700">Sin configurar</span>}
                  </td>
                  <td className="px-4 py-4">
                    {record.workerType ? (
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-700"><BriefcaseBusiness className="h-3.5 w-3.5" />{WORKER_TYPE_LABELS[record.workerType]}</span>
                    ) : <span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700">Sin definir</span>}
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex max-w-72 flex-wrap gap-1.5">
                      {record.stores.length > 0 ? record.stores.map((store) => (
                        <span key={store.id} className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700"><Store className="h-3 w-3" />{store.code} · {store.name}</span>
                  ...
                  <td className="px-4 py-4">
                    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${record.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>
                      {record.active ? <CheckCircle2 className="h-3.5 w-3.5" /> : <Ban className="h-3.5 w-3.5" />}{record.active ? "Activo" : "Inactivo"}
                    </span>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex justify-end gap-2">
                      {record.canEdit && record.role && record.position && (
                        <button type="button" onClick={() => openEdit(record)} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 px-2.5 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-100"><Edit3 className="h-3.5 w-3.5" />Editar</button>
                      )}
                      {record.canToggleActive && (
                        <button type="button" onClick={() => toggleActive(record)} disabled={isPending} className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-2 text-xs font-semibold transition disabled:opacity-50 ${record.active ? "border border-rose-200 text-rose-700 hover:bg-rose-50" : "border border-emerald-200 text-emerald-700 hover:bg-emerald-50"}`}>
                          {record.active ? <Ban className="h-3.5 w-3.5" /> : <CheckCircle2 className="h-3.5 w-3.5" />}{record.active ? "Desactivar" : "Reactivar"}
                        </button>
                      )}
              ...
              {filteredStaff.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-12 text-center"><UserRound className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 font-medium text-slate-700">No se encontró personal</p></td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        >
          <div className="max-h-[94vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
            <div className="sticky top-0 z-10 flex items-start justify-between border-b border-slate-200 bg-white px-5 py-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-blue-700">{dialog.mode === "create" ? "Alta de personal" : "Edición de personal"}</p>
                <h2 className="mt-1 text-xl font-bold text-slate-950">{dialog.mode === "create" ? "Nuevo personal" : dialog.record.fullName}</h2>
              </div>
              <button type="button" onClick={() => void requestClose()} disabled={isPending} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 disabled:opacity-50" aria-label="Cerrar"><X className="h-5 w-5" /></button>
            </div>

            <form onSubmit={submitForm} className="space-y-6 p-5">
              {editingSelf && (
                <div className="flex gap-3 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" />Puedes actualizar tus datos de acceso, pero no tu propio rol, cargo, tipo de trabajador, estado ni alcance de tiendas.</div>
              )}

              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Nombre completo" error={errors.fullName?.message} wide>
                  <input {...register("fullName", { required: "El nombre es obligatorio", minLength: { value: 3, message: "Ingresa al menos 3 caracteres" } })} className="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" placeholder="Nombres y apellidos" />
                </Field>
                ...
                <Field label="DNI" error={errors.dni?.message}>
                  <input inputMode="numeric" maxLength={8} {...register("dni", { required: "El DNI es obligatorio", pattern: { value: /^\d{8}$/, message: "El DNI debe tener 8 dígitos" } })} className="w-full rounded-xl border border-slate-300 px-3 py-2.5 font-mono text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" placeholder="12345678" />
                </Field>
                <Field label="Cargo">
                  <select {...register("position", { required: true })} disabled={editingSelf} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none disabled:bg-slate-100 disabled:text-slate-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
                ...
                <Field label="Rol del sistema">
                  <select {...register("role", { required: true })} disabled={editingSelf} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none disabled:bg-slate-100 disabled:text-slate-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
                ...
                <Field label="Tipo de trabajador" error={errors.workerType?.message} wide>
                  <select {...register("workerType", { required: dialog.mode === "create" || !editingSelf ? "Selecciona Full Time o Part Time" : false })} disabled={editingSelf} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none disabled:bg-slate-100 disabled:text-slate-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
                ...
                <Field label={dialog.mode === "create" ? "Contraseña temporal" : "Nueva contraseña"} error={errors.password?.message} wide>
                  <input type="password" autoComplete="new-password" {...register("password", { required: dialog.mode === "create" ? "La contraseña temporal es obligatoria" : false, validate: (value) => !value || value.length >= 8 || "Usa al menos 8 caracteres" })} className="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" placeholder={dialog.mode === "create" ? "Mínimo 8 caracteres" : "Déjala vacía para conservarla"} />
              ...
              <fieldset disabled={editingSelf} className="rounded-2xl border border-slate-200 p-4 disabled:opacity-60">
                ...
                <p className="mb-3 text-xs text-slate-500">{storeRequired ? "Este cargo requiere al menos una tienda." : "La asignación de tiendas es opcional para este rol/cargo."}</p>
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    {initialData.stores.map((store) => (
                      <label key={store.id} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 transition ${selectedStores.includes(store.id) ? "border-blue-300 bg-blue-50" : "border-slate-200 hover:bg-slate-50"}`}>
                        ...
                        <span><span className="block text-sm font-medium text-slate-800">{store.name}</span><span className="mt-0.5 block text-xs text-slate-500">{store.code}</span></span>
                      </label>
                    ))}
                  </div>
                ) : <div className="rounded-xl bg-amber-50 p-3 text-sm text-amber-800">No hay tiendas activas disponibles dentro de tu alcance.</div>}
                {errors.storeIds && <p className="mt-2 text-xs text-rose-600">{errors.storeIds.message}</p>}
              </fieldset>

              {isDirty && (
                <div className="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" />Hay cambios sin guardar. Si intentas cerrar el modal se solicitará confirmación.</div>
              )}

              <div className="flex flex-col-reverse gap-2 border-t border-slate-200 pt-4 sm:flex-row sm:justify-end">
                <button type="button" onClick={() => void requestClose()} disabled={isPending} className="rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50">Cancelar</button>
                <button type="submit" disabled={isPending} className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">
                  {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />}{dialog.mode === "create" ? "Crear personal" : "Guardar cambios"}
                </button>
              </div>
            </form>
          </div>
...
function Metric({ label, value, tone = "slate" }: { label: string; value: number; tone?: "slate" | "green" | "blue" | "violet" }) {
  ...
  return <div className={`rounded-2xl border p-4 shadow-sm ${tones[tone]}`}><p className="text-xs font-semibold uppercase tracking-wide opacity-70">{label}</p><p className="mt-2 text-2xl font-bold">{value}</p></div>;
}

function Field({ label, error, wide = false, children }: { label: string; error?: string; wide?: boolean; children: React.ReactNode }) {
  return <label className={wide ? "sm:col-span-2" : undefined}><span className="mb-1.5 block text-sm font-medium text-slate-700">{label}</span>{children}{error && <p className="mt-1 text-xs text-rose-600">{error}</p>}</label>;
}
